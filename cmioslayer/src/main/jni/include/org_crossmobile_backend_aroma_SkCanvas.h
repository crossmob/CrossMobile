/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class org_crossmobile_backend_aroma_SkCanvas */

#ifndef _Included_org_crossmobile_backend_aroma_SkCanvas
#define _Included_org_crossmobile_backend_aroma_SkCanvas
#ifdef __cplusplus
extern "C" {
#endif
#undef org_crossmobile_backend_aroma_SkCanvas_MITER_JOIN
#define org_crossmobile_backend_aroma_SkCanvas_MITER_JOIN 0L
#undef org_crossmobile_backend_aroma_SkCanvas_ROUND_JOIN
#define org_crossmobile_backend_aroma_SkCanvas_ROUND_JOIN 1L
#undef org_crossmobile_backend_aroma_SkCanvas_BEVEL_JOIN
#define org_crossmobile_backend_aroma_SkCanvas_BEVEL_JOIN 2L
#undef org_crossmobile_backend_aroma_SkCanvas_LAST_JOIN
#define org_crossmobile_backend_aroma_SkCanvas_LAST_JOIN 2L
#undef org_crossmobile_backend_aroma_SkCanvas_DEFAULT_JOIN
#define org_crossmobile_backend_aroma_SkCanvas_DEFAULT_JOIN 0L
#undef org_crossmobile_backend_aroma_SkCanvas_BUTT_CAP
#define org_crossmobile_backend_aroma_SkCanvas_BUTT_CAP 0L
#undef org_crossmobile_backend_aroma_SkCanvas_ROUND_CAP
#define org_crossmobile_backend_aroma_SkCanvas_ROUND_CAP 1L
#undef org_crossmobile_backend_aroma_SkCanvas_SQUARE_CAP
#define org_crossmobile_backend_aroma_SkCanvas_SQUARE_CAP 2L
#undef org_crossmobile_backend_aroma_SkCanvas_LAST_CAP
#define org_crossmobile_backend_aroma_SkCanvas_LAST_CAP 2L
#undef org_crossmobile_backend_aroma_SkCanvas_DEFAULT_CAP
#define org_crossmobile_backend_aroma_SkCanvas_DEFAULT_CAP 1L
/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    initSDLWindowCanvas
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_initSDLWindowCanvas
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    initBitmapCanvas
 * Signature: (J)J
 */
JNIEXPORT jlong JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_initBitmapCanvas
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    destroyCanvas
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_destroyCanvas
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    clear
 * Signature: (JI)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_clear
  (JNIEnv *, jclass, jlong, jint);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    flush
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_flush
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawText
 * Signature: (JDDLjava/lang/String;JJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawText
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jstring, jlong, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawRect
 * Signature: (JDDDDJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawRect
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jdouble, jdouble, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawImage
 * Signature: (JJDDDD)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawImage
  (JNIEnv *, jclass, jlong, jlong, jdouble, jdouble, jdouble, jdouble);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawLine
 * Signature: (JDDDDJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawLine
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jdouble, jdouble, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawRRect
 * Signature: (JDDDDJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawRRect
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jdouble, jdouble, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawArc
 * Signature: (JDDDDDDJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawArc
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jdouble, jdouble, jdouble, jdouble, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawPath
 * Signature: (JJJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawPath
  (JNIEnv *, jclass, jlong, jlong, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    save
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_save
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    restore
 * Signature: (J)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_restore
  (JNIEnv *, jclass, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    translate
 * Signature: (JDD)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_translate
  (JNIEnv *, jclass, jlong, jdouble, jdouble);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    rotate
 * Signature: (JD)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_rotate
  (JNIEnv *, jclass, jlong, jdouble);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    scale
 * Signature: (JDD)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_scale
  (JNIEnv *, jclass, jlong, jdouble, jdouble);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    clipRect
 * Signature: (JDDJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_clipRect
  (JNIEnv *, jclass, jlong, jdouble, jdouble, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    fill
 * Signature: (JJ)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_fill
  (JNIEnv *, jclass, jlong, jlong);

/*
 * Class:     org_crossmobile_backend_aroma_SkCanvas
 * Method:    drawNinePatch
 * Signature: (JJIIIIII)V
 */
JNIEXPORT void JNICALL Java_org_crossmobile_backend_aroma_SkCanvas_drawNinePatch
  (JNIEnv *, jclass, jlong, jlong, jint, jint, jint, jint, jint, jint);

#ifdef __cplusplus
}
#endif
#endif
