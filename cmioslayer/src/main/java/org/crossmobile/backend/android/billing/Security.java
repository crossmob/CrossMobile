...
public class Security {
    ...
    public static PublicKey generatePublicKey(String encodedPublicKey) {
        try {
            byte[] decodedKey = MiGBase64.decode(encodedPublicKey);
            if (decodedKey == null)
                throw new IllegalArgumentException("Not valid data to decode");
    ...
    public static boolean verify(PublicKey publicKey, String signedData, String signature) {
        ...
        try {
            ...
            byte[] dsugnature = MiGBase64.decode(signature);
            if (dsugnature == null) {
                Log.e(TAG, "Base64 decoding failed.");
                return false;
            }
            if (!sig.verify(dsugnature)) {
